...
test("a job's timeoutMs and model are passed through to runAgent", async () => {
  const h = harness([job({ timeoutMs: 30000, model: "opus-x" })]);
  await h.scheduler.start();
  h.active()[0].fn();
  await flush();
  assert.equal((h.runs[0] as { timeoutMs?: number }).timeoutMs, 30000);
  assert.equal((h.runs[0] as { modelOverride?: string }).modelOverride, "opus-x");
});
